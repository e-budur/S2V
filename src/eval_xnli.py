...
def evaluate(encoder, seed=1234, evaltest=False, loc='./data/', file_meta_data=None, sp=None):
    ...
    devA = encoder.encode(dev[0], verbose=False, use_eos=True, max_sent_len=100)
    devB = encoder.encode(dev[1], verbose=False, use_eos=True, max_sent_len=100)
    ...
    testY = encode_labels(labels[2], label_encoder)
    ...
    if evaltest:
        ...
        testA = encoder.encode(test[0], verbose=False, use_eos=True, max_sent_len=100)
        testB = encoder.encode(test[1], verbose=False, use_eos=True, max_sent_len=100)
...
def train_model(lrmodel, X, Y, devX, devY):
    """
    Train model, using val_loss on dev for early stopping
    """
...
def load_data(loc='./data/', file_meta_data=None, sp=None):
    ...
    language = file_meta_data['language']
    ...
    with codecs.open(os.path.join(loc, file_meta_data['file_names']['dev']), mode='rb', encoding='utf-8') as f:
        for item in json_lines.reader(f):
           if item['gold_label'] == '-':
              continue
           if not (language == 'any' or language == item['language']):
              continue
